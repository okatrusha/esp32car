...
while True:
    try:
        ...
        cv2.imshow("ESP ball Chase", frame)
        ...
        for r in results:
            ...
            for box in r.boxes:
                ...
                if (label in ["sports ball"] and conf > 0.2):
                    ball_found = True
                    send_cmd("stop")
                    break
        ...
        if ball_found == True:
            send_cmd("stop")
            if searching:
                print("🎯 Ball found — exit search mode")
                searching = False
                # send_cmd("right")
                # send_cmd("stop")
                # time.sleep(0.2)
            ...
            if searching:                
                ...
                # stop if too long without finding ball
